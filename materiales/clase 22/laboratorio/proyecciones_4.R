...
################################################################################
# El objetivo de este laboratorio es agregar algunas herramientas de visualización
# a las proyecciones de población que veníamos trabajando. Específicamente, la 
# idea es visualizar el cambio en el tamaño de la población a medida que vamos
# proyectando, y observar el cambio en la estructura por edad y sexo de la población
# con un gráfico de "pirámide" de población.

# Las funciones para preparar los datos y graficar se definen a continuación:
...
# Ejercicio: Completar la función para proyectar la población y graficar resultados 
p_pop_base <- function(f, Lf, Lm, Nf, Nm, iter, ini, int, srb){
  ...
  # Años
  Y <- as.numeric(seq(ini, ini + (int*iter), int))
  ...
  for(i in is){
    ...
    Sys.sleep(0.1)
    # pasar la población proyectada en la iteración a formato long
    dat_long <- reshape_long(dat = , ini, int)
    
    # Guardar el tamaño total de la población proyectada en la iteración
    gw$N[idx] <- 
