...
SRB <- 1.05                     # Razón de masculinidad al nacer
prop_fem <- 1 / (1 + SRB)       # Proporción niñas
prop_masc <- SRB / (1 + SRB)    # Proporción niños
radix <- 100000                 # Raíz de la tabla 
...
# Proyectamos la población femenina usando la matriz (en el próximo paso corregimos la 
# primera entrada)
Mlf <- 
swf$p98 <- 
...
p_pop <- function(f, Lf, Lm, Nf, Nm, iter, int, srb){
  ...
  pop <- matrix(NA,length(Lf), (iter+1)*2)
  pop[,1] <- Nf
  pop[,2] <- Nm
